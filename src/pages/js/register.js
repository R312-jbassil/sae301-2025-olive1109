...
export const POST = async ({ request, cookies }) => {
    ...
    try {
        ...
        const authData = await pb.collection(Collections.Users).create(email, password);

        console.log('/js/register - auth succeeded, user id:', authData.record?.id);
        console.log('/js/register - pb.authStore.isValid:', pb.authStore.isValid);
        console.log('/js/register - pb.authStore.record?.id:', pb.authStore.record?.id);
